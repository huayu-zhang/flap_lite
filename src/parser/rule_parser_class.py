...
class RuleParser:
    ...
    def __init__(self, sql_db):
        ...
        self.l_dl_names = self.load_json(sql_db.sub_paths['unique_DEPENDENT_LOCALITY'])
        self.l_ddl_names = self.load_json(sql_db.sub_paths['unique_DOUBLE_DEPENDENT_LOCALITY'])
        ...
        self.regex_rules = {
            ...
            'THOROUGHFARE': [
                r'(%s) ((?:[A-Z]+ ){1,3}(?:%s))(?:\s|$|,|\n)' % ('\d+', '|'.join(self.l_thoroughfare_names)),
                r'(%s) ((?:[A-Z]+ ){1,3}(?:%s))(?:\s|$|,|\n)' % ('\d+[A-Z]+', '|'.join(self.l_thoroughfare_names)),
                r'(%s) ((?:[A-Z]+ ){1,3}(?:%s))(?:\s|$|,|\n)' % ('[A-Z]+\d+', '|'.join(self.l_thoroughfare_names)),
                r'(%s) ((?:[A-Z]+ ){1,3}(?:%s))(?:\s|$|,|\n)' % ('\d+(?:/|-)\d+', '|'.join(self.l_thoroughfare_names)),
                r'(%s) ((?:[A-Z]+ ){1,3}(?:%s))(?:\s|$|,|\n)' % ('\d+(?:/|-)[A-Z]+', '|'.join(self.l_thoroughfare_names)),
                r'(%s) ((?:[A-Z]+ ){1,3}(?:%s))(?:\s|$|,|\n)' % ('[A-Z]+(?:/|-)\d+', '|'.join(self.l_thoroughfare_names)),
                r'(%s) ((?:[A-Z]+ ){1,3}(?:%s))(?:\s|$|,|\n)' % ('[A-Z]+(?:/|-)[A-Z]+', '|'.join(self.l_thoroughfare_names)),
    ...
    def number_like_parsing(self, address):
        ...
        l_match = self._pattern_mapping(self.number_like_regex['MULTIPLIER'], address, keep='first')
        ...
        if len(l_match):
            ...
            match = re.search(self.number_like_regex['NUMBER_LIKE'][0], residual_address)
        ...
        l_match = self._pattern_mapping(self.number_like_regex['NUMBER_COMPOUND'], address, keep='first')
        ...
        l_match = self._pattern_mapping(self.number_like_regex['NUMBER_SECONDARY_PREFIX'], address)
        ...
        l_match = self._pattern_mapping(self.number_like_regex['NUMBER_LIKE'], address, keep='all')
    ...
    def parse(self, address):
        ...
        parsed = self.exact_parsing(address)

        if parsed['need_fuzzy_parsing']:
            parsed = self.fuzzy_parsing(address)

        parsed_number_like = self.number_like_parsing(parsed['residual_address'])
